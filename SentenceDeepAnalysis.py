...
class SentenceDeepAnalysis (object):
  ...
  time_units = [u'年',u'月',u'天',u'日',u'次',u'笔',u'星期']
  eng_time_units = ['y','m','d','d','t','t','w']
...
#      pos = i
#      while j<p_len:
#        if pos+j < s_len and sarr[pos+j] == pparr[j]:
#          pos = pos + 1
#          j = j + 1
#          if j == p_len:
#            return i
...
#    return -1
  ...
  def __isMatch (self,sarr,pparr):
    ...
    while (i <= s_len - p_len):
      ...
      if sarr[i] == pparr[j]:
        j = j + 1
        while (j < p_len and sarr[i+j] == pparr[j]):
          j = j + 1
          if (j == p_len):
            return i
      i = i + 1
  ...
  def __iscontains (self,sarr,parr):
    for pparr in parr:
      sarr_pos = self.__isMatch(sarr,pparr)
      if sarr_pos != -1:
        return (sarr_pos,pparr)
    return (-1,None);

  def __getQua_Unit (self,pos,words):
    s_pos , pattern = self.__iscontains(pos,self.time_pat)
    #print (s_pos)
    #print (pattern)
    if not s_pos == -1 and not pattern is None: 
      i = j = k = 0
      m_word = q_word = None
      while i < len(pattern):
        if pattern[i] == 'm':
          m_word = words[s_pos + i]
          #print (m_word)
        if pattern[i] == 'n' or pattern[i] == 'nd':
          q_word = words[s_pos + i]
          #print (q_word)
        i = i + 1
      #if m_word is None:
      #  while k < len(pattern):
      ...
      #      if words[s_pos +k] == '上':
      #        m_word = '一'
      if not m_word is None and not q_word is None :
        #print ("准备获取数字")
        m_num = self.__mapWordNum(m_word)
        #print (m_num)
        #print ("准备获取单位")
        q_num = self.__getTimeUnit(q_word)
        #print (q_num)
        return [m_num,q_num]
      else :
        return ["None", "无法定位到您想要的时间点"]	
    return ["None", "无法获取匹配关系"]
  ...
  def __getTimeInfo (self, words, pos):
    ...
    if (len(nd_words) > 1 or len(nt_words) > 1):
     return ["None", "无法定位到您想要的时间点1"]
    
    #if not nt_words is None and nt_words[0] == "上个月":
    # return [1,m] 
    ...
    for sentence in sentences:
      ...
      if len(person) == 0:
        ...
        for verb in verbs:
          ...
          #print ("verbs " + verb + " is in frwd_words" )
          ...
          if vob_relation_id : 
            ...
            for id in vob_relation_id :
              vob_relation_head = self.__getRelationHead(sentence, arcs, id)
              if (sentence[id].find(u'元')!= -1 or sentence[id].find(u'块')!= -1 or sentence[id].isdigit())and vob_relation_head == verb:
                frwd_amount = self.__checkFrwdAmount(sentence,pos)
                #print (frwd_amount)
                if len(frwd_amount) != 1 :
                  return [0, "缺失转账金额或没有转账金额"]
        ...
        if pob_relation_id :
          ...
          #print ("POB on " + str(pob_relation_id))
          #print ("OBJ is " + str(sentence[pob_relation_id[0]]))
          if (sentence[pob_relation_id[0]].find(u'元')!= -1 or sentence[pob_relation_id[0]].find(u'块')!= -1 or sentence[pob_relation_id[0]].isdigit())and pob_relation_head == pls[0]:
            frwd_amount = self.__checkFrwdAmount(sentence,pos)
            #print (frwd_amount)
            if len(frwd_amount) != 1 :
              return [0, "缺失转账金额或没有转账金额"]
  ...
  def checkAccountExch (self,sentences): 
    for sentence in sentences:
      ...
      if (len(person) > 1):
        return [0, "你不可能查两个人的交易记录"]
      if (len(person) != 0 and person[0] != self.username):
        return [0, "你不可能查别人的交易记录"]
      verbs = self.__getVerbinSent(sentence,pos)
      advs = self.__getAdvinSent(sentence,pos)
      if self.rela_vob in arc_relations and verbs:
        for verb in verbs:
          ...
          if (sentence[id] in self.exch_att_1 and sbv_relation_head in self.exch_att_2) or \
            ...
            for pat in self.time_pos_pat:
              ...
              if self.__ifSeqinSeq(sentence_pat,seq_pat) != "None":
                ...
                #print (time_result)
                return [2,time_result[0],time_result[1]]
      return [0,"指令不对，无法完成收入查询操作"]          
    return [0,"指令不明确，无法完成收入查询操作"]    
